  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/scans/receive', {
        ...
        body: JSON.stringify({ ...formData, scan_payload: formData.asset_tag, user_id: 'tech-jane' }),
      });
      router.push('/manager');
    } catch (err: any) {
  ...
  return (
    <ScanWorkflowShell title="Receive Asset" error={error}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <ScanField label="Serial" value={formData.serial} onChange={(v) => setFormData(p => ({...p, serial: v}))} onScan={() => setScanningField('serial')} />
        <ScanField label="Model" value={formData.model} onChange={(v) => setFormData(p => ({...p, model: v}))} onScan={() => setScanningField('model')} />
        <ScanField label="Manufacturer" value={formData.manufacturer} onChange={(v) => setFormData(p => ({...p, manufacturer: v}))} onScan={() => setScanningField('manufacturer')} />
        <select 
          value={formData.asset_class}
          onChange={(e) => setFormData({...formData, asset_class: e.target.value})}
          className="w-full border border-gray-300 rounded-lg p-2.5"
        >
          {['instrument', 'compute', 'network', 'power', 'consumable_durable'].map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <button type="submit" className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold">Submit Asset</button>
      </form>
    </ScanWorkflowShell>
  );
};
